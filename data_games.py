...
def process_amazon_data(file_path):
    interactions = pd.read_csv(f"{file_path}.inter", sep='\t')
    items = pd.read_csv(f"{file_path}.item", sep='\t', encoding='utf-8')
    ...
    initial_interaction_count = len(interactions)
    interactions = interactions[interactions['item_id'].isin(valid_items)]
    filtered_interaction_count = len(interactions)
    
    unique_users = interactions['user_id'].unique()
    user_id_map = {old_id: new_id for new_id, old_id in enumerate(unique_users, 1)}
    
    interacted_items = interactions['item_id'].unique()
    item_id_map = {old_id: new_id for new_id, old_id in enumerate(interacted_items, 1)}
    
    interactions['new_user_id'] = interactions['user_id'].map(user_id_map)
    interactions['new_item_id'] = interactions['item_id'].map(item_id_map)
    
    items = items[items['item_id'].isin(interacted_items)]
    items['new_item_id'] = items['item_id'].map(item_id_map)
    
    new_interactions = interactions[['new_user_id', 'new_item_id', 'rating', 'timestamp']]
    new_interactions = new_interactions.rename(columns={
        'new_user_id': 'user_id',
        'new_item_id': 'item_id'
    })
    
    new_items = items[['new_item_id', 'title']]
    new_items = new_items.rename(columns={'new_item_id': 'item_id'})
    
    new_interactions.to_csv(f"{file_path}_processed.inter", sep='\t', index=False)
    new_items.to_csv(f"{file_path}_processed.item", sep='\t', index=False)
    
    user_map_df = pd.DataFrame({
        'original_user_id': list(user_id_map.keys()),
        'new_user_id': list(user_id_map.values())
    })
    item_map_df = pd.DataFrame({
        'original_item_id': list(item_id_map.keys()),
        'new_item_id': list(item_id_map.values())
    })
    user_map_df.to_csv(f"{file_path}_user_id_mapping.csv", index=False)
    item_map_df.to_csv(f"{file_path}_item_id_mapping.csv", index=False)
    
    return new_interactions, new_items

def sort_data(file_path):
    df = pd.read_csv(f"{file_path}_processed.inter", sep='\s+')
...
def get_data(file_path):
    df = pd.read_csv(f"{file_path}.sorted", sep='\s+', usecols=[0, 1, 3], names=['user_id', 'item_id', 'timestamp'])
...
def generate_candidate(file_path, test_data, num_candidate):
    ...
    df = pd.read_csv(f"{file_path}.sorted", sep=' ', usecols=[0, 1, 3], names=['user_id', 'item_id', 'timestamp'])
    ...
    for user_id in candidate_dict:
        if int(user_id) not in test_pos or int(user_id) not in user_history:
            continue
        user_list.append(user_id)
        interacted = user_history[int(user_id)]
...
def load_text(file_path):
    item_name = {}
    with open(f"{file_path}_processed.item", 'r', encoding='utf-8') as f:
        f.readline()
        for line in f:
            v = line.strip().split('\t')
            item_name[v[0]] = v[1]
