...
def sort_data(file_path):
    df = pd.read_csv(f"{file_path}.inter", sep='\s+', names=['user_id', 'item_id', 'rating', 'timestamp'])
...
def get_data(file_path):
    df = pd.read_csv(f"{file_path}.sorted", sep='\s+', names=['user_id', 'item_id', 'rating', 'timestamp'])
    
    train_data, valid_data, test_data = [], [], []
    for user_id, group in df.groupby('user_id'):
        group_sorted = group.sort_values('timestamp') 
        if len(group_sorted) < 2: continue  # 跳过不足2条的用户
        
        test = group_sorted.iloc[-1:]
        valid = group_sorted.iloc[-2:-1]
        # train = group_sorted.iloc[:-2]
        train = group_sorted.iloc[:-1] # zero-shot场景验证集被合并
...
def get_candidate(file_path, num_user):
    ...
    for user in selected_users:
        item_list = random.sample(list(all_items), 100)  # 随机选100个物品
...
def generate_candidate(file_path, test_data, num_candidate):
    ...
    df = pd.read_csv(f"{file_path}.sorted", sep=' ', names=['user_id', 'item_id', 'rating', 'timestamp'])
