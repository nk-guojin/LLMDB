...
class LLMDB(BaseModel):
    ...
    def request_api_iter(self, prompt_list, iter_list, last_reply):
        request_list = []
    ...
    def build_prompt_his(self, user_his_name, candidates_name):
        if (self.dataset == "lastfm"):
            prompt_nir = "You are a music recommender system.\n" \
    ...
    def build_prompt_prefer(self, user_his_name, candidates_name, user_his_answer):
        if (self.dataset == "lastfm"):
    ...
    def build_prompt_rec(self, user_his_name, candidates_name, act_answer, cri_answer):
        if (self.dataset == "lastfm"):
            prompt_rec = f"You are a music recommender system.\n" \
                    ...
                    f"Based on former recommendations and improvement suggestions, rerank these {len(candidates_name)} artists by measuring the possibilities that I would like to listen next most, according to my listen history. Please think step by step.\n" \
            ...
            prompt_rec = f"You are a game recommender system.\n" \
                    ...
                    f"{act_answer}\n" \
                    f"Given the feedback of recommendation: {cri_answer}.\n" \
                    f"Based on former recommendations and improvement suggestions, rerank these {len(candidates_name)} games by measuring the possibilities that I would like to play next most, according to my play history. Please think step by step.\n" \
                    f"Please show me your ranking results with order numbers. Split your output with line break. Rank directly, you can only generate game names. You MUST rank the given candidate games. You can not generate games that are not in the given candidate list."
        return prompt_rec

    def get_score(self, candidate_text, iter_list, prefer_list):
        score = 0
        alpha = 0.6
        wh = 0
        for i in range(len(iter_list)):
            if iter_list[i] == candidate_text:
                wh = 1 - i / len(iter_list)
                score += wh * (len(iter_list) - i)
                break

        for i in range(len(prefer_list)):
            if prefer_list[i] == candidate_text:
                score += (1 - wh) * (len(prefer_list) - i)                     
                break
    ...
    def predict_rank(self):
        ...
        rec_list_1 = self.request_api_iter(prompt_list_act_list[0], iter_list, last_reply_act)
        ...
        for it in range(0, 3):
            ...
            cri = self.request_api_iter(prompt_list_cri_list[it], iter_list, last_reply_cri)
            cri_list.append(cri)
            last_reply_cri = cri
            ...
            rec_list_2 = self.request_api_iter(prompt_list_act_list[it + 1], iter_list, last_reply_act)
        ...
        request_list_his = self.request_api(prompt_list_his)

        for i in range(self.batch_size):
            user_his_name = user_his_name_list[i]
            candidates_name = candidates_name_list[i]
            prompt_prefer = self.build_prompt_prefer(user_his_name, candidates_name, request_list_his[i])
            prompt_list_prefer.append(prompt_prefer)
        request_list_prefer = self.request_api(prompt_list_prefer)

        iter_text_list = []
        for request_text in req_list[it_count]:
            iter_text = self.parse_text(request_text)
            iter_text_list.append(iter_text)

        prefer_text_list = []
        for request_text in request_list_prefer:
            prefer_text = self.parse_text(request_text)
            prefer_text_list.append(prefer_text)

        predict_text_list = []

        for i in range(self.batch_size):
            candidate_sorted = sorted(candidates_name_list[i], key=lambda x: self.get_score(x, iter_text_list[i], prefer_text_list[i]), reverse=True)
            predict_text_list.append(candidate_sorted)
        ...
        for i in range(self.batch_size):
            predict_score = []
            for j in range(len(predict_text_list[i])):
                predict_score.append((predict_text_list[i][j], self.get_score(predict_text_list[i][j], iter_text_list[i], prefer_text_list[i])))
        ...
        for i in range(self.batch_size):
            ...
            result_json["Input_his"] = prompt_list_his[i]
            result_json["Input_prefer"] = prompt_list_prefer[i]
            ...
            result_json["Predict_his"] = request_list_his[i]
            result_json["Predict_prefer"] = request_list_prefer[i]
            result_json["Predict_merge"] = predict_score_list[i]
            result_json["hit_num"] = predict_num_list[i] + 1
            results_data.append(result_json)

        file_dir = f"./logs/{self.dataset}/LLMDB_{self.dataset}.json"
